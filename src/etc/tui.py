...
def list_logs(logs_dir):
    """Return (label, path) for every .csv under logs_dir, newest first.

    Walks category subdirectories (e.g. data/raw/circle/…) so the label shows
    the gesture folder: "circle/circle".
    """
    found = []
    for root, _dirs, files in os.walk(logs_dir):
        for f in files:
            if f.endswith(".csv"):
                ...
                found.append((rel[:-4], path, os.path.getmtime(path)))
